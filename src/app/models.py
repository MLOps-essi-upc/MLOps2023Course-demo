...
class LMBERTModel(Model):
    ...
    def predict(self, user_input: str, n=5):
        ...
        model = BertForMaskedLM.from_pretrained(
            pretrained_model_name_or_path="bert-base-uncased", return_dict=True
        )  # ./bert-base-uncased
        # return_dict True to use mask token

        response = {"prediction": self.infer(user_input, model, tokenizer)}
        return response

    @track_emissions(
        project_name="bert", output_file=RESULTS_DIR + "emissions_bert.csv"
    )
    def infer(self, text: str, model, tokenizer) -> str:
        # tokenize
...
class T5Model(Model):
    ...
    def predict(self, user_input: str):
        ...
        model = T5ForConditionalGeneration.from_pretrained(
            "t5-small", low_cpu_mem_usage=True
        )
...
class CodeGenModel(Model):
    ...
    @track_emissions(
        project_name="codegen", output_file=RESULTS_DIR + "emissions_codegen.csv"
    )
    def infer(self, text: str, model, tokenizer) -> str:
        # tokenize
        inputs = tokenizer(text, return_tensors="pt")
...
class Pythia_70mModel(Model):
    ...
    @track_emissions(
        project_name="pythia", output_file=RESULTS_DIR + "emissions_pythia.csv"
    )
    def infer(self, text: str, model, tokenizer) -> str:
        # tokenize
        inputs = tokenizer(text, return_tensors="pt")
...
class Codet5p_220mModel(Model):
    ...
    @track_emissions(
        project_name="codet5p", output_file=RESULTS_DIR + "emissions_codet5p.csv"
    )
    def infer(self, text: str, model, tokenizer) -> str:
